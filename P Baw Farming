...
local FastRebTab = window:AddTab("รีไว")
...
local serverLabel = FastRebTab:AddLabel("เวลา:")
...
FastRebTab:AddSwitch("เริ่มรีไว", function(state)
    isRunning = state
...
FastRebTab:AddButton("จอดำ", function()
    local player = game.Players.LocalPlayer
...
local miscLabel = FastRebTab:AddLabel("เพิ่มเติม:")
miscLabel.TextSize = 20

local lockRunning = false
local lockThread = nil

local lockSwitch = FastRebTab:AddSwitch("ล็อคตำแหน่ง", function(state)
    lockRunning = state
    if lockRunning then
        local player = game.Players.LocalPlayer
...
local autoshakeSwitch = FastRebTab:AddSwitch("กินน้ำ/ขนม ออโต้", function(state)
    running = state
    if state then
        activateShake()
    end
...
local spinwheelSwitch = FastRebTab:AddSwitch("ออโต้สปิน", function(bool)
...
local FarmingTab = window:AddTab("ฟามไว")
...
FarmingTab:AddLabel("เวลา:").TextSize = 20
...
FarmingTab:AddLabel("")
FarmingTab:AddLabel("ฟามไว (แนะนำตั้งเลข: 20-25)").TextSize = 20
...
FarmingTab:AddTextBox("ความเร็วในการยก", function(value)
...
FarmingTab:AddSwitch("เริ่มฟามไว", function(state)
...
FarmingTab:AddLabel("เพิ่มเติม:").TextSize = 20
...
local switch = FarmingTab:AddSwitch("ออโต้กินไข่ทุกๆ30นาที", function(state)
    running = state
    if state then
        activateProteinEgg()
    end
...
local switch = FarmingTab:AddSwitch("ออโต้กินน้ำ", function(state)
    running = state
    if state then
        activateShake()
    end
...
local spinwheelSwitch = FarmingTab:AddSwitch("ออโต้สปิน", function(bool)
    _G.AutoSpinWheel = bool
...
FarmingTab:AddButton("จอดำ", function()
    local player = game.Players.LocalPlayer
    local playerGui = player:WaitForChild("PlayerGui")
    local lighting = game:GetService("Lighting")
...
FarmingTab:AddButton("ใส่ตัวซามูไรทั้งหมด", function()
    unequipPets()
